...
#define worryType uint64_t
//#define worryType int64_t
//#define worryType __uint128_t
//#define worryType double

typedef struct monkey {
  vector<worryType> worryList;
  char oper;
  int operValue;
  int divisValue;
  int trueTo;
...
vector<monkey_t> monkeyList;

void zeroMonkey(monkey_t* pM) {
    pM->worryList.clear();
    pM->oper='X';
    pM->operValue=-1;
    pM->divisValue=-1;
...
void day11parse(vector<string>& inList) {
  monkey_t theMonkey;

  zeroMonkey(&theMonkey);

  for(auto i=0; i<inList.size(); i++) {
    if (inList[i].length()==0) {
      // Record is complete.
      monkeyList.push_back(theMonkey);
      zeroMonkey(&theMonkey);
      continue;
    }

    char first[20];
    sscanf(inList[i].c_str(), "%s", first);
    printf("[%d]: First=%s\n", i, first);
    ...
    if (strFirst=="Monkey") {
    ...
    else if (strFirst=="Starting") {
      // Iterate through the list of integers at the end.
      size_t cur=inList[i].find(':');
      assert(cur>0 || cur==std::string::npos);
      cur += 2;  // Skip over the ": "
      ...
      while (1) {
        ...
        sscanf(inList[i].c_str()+cur, "%d", &tmpWorry);
        theMonkey.worryList.push_back(tmpWorry);
        printf("[%d]:   Got worry of: %d\n", i, tmpWorry);

        cur = inList[i].find(' ', cur+1);
      }
    }
    else if (strFirst=="Operation:") {
      size_t cur = inList[i].find("old");
      assert(cur>0 || cur==std::string::npos);
      cur += 4;  // Skip over the "old "

      theMonkey.oper = inList[i][cur];
      cur += 2;   // Skip operator and space

      sscanf(inList[i].c_str()+cur, "%d", &theMonkey.operValue);
      printf("[%d]:   new = old %c %d\n", i, theMonkey.oper, theMonkey.operValue);
    }
    else if (strFirst=="Test:") {
      size_t cur = inList[i].find("by");
      assert(cur>0 || cur==std::string::npos);
      cur += 3;  // Skip over the "by "

      sscanf(inList[i].c_str()+cur, "%d", &theMonkey.divisValue);
      printf("[%d]:   Test: div by %d\n", i, theMonkey.divisValue);
    }
    else if (strFirst=="If") {
      bool bUseTrue=false;
      int tmpTo;
      size_t cur = inList[i].find("true");
      if (cur != std::string::npos) {
        // True side.
        bUseTrue = true;
      }

      cur = inList[i].find("monkey");
      assert(cur>0 || cur==std::string::npos);
      cur += 7;  // Skip over the "monkey "

      sscanf(inList[i].c_str()+cur, "%d", &tmpTo);
      printf("[%d]:   True/False goes to %d\n", i, tmpTo);
      if (bUseTrue)
        theMonkey.trueTo = tmpTo;
      else
        theMonkey.falseTo = tmpTo;
    }
  }
}

void day11printMonkies() {
  printf("Monkey List::\n");
  for (auto i=0; i<monkeyList.size(); i++) {
    printf("M:%d: [", i);
    for(auto j=0;j<monkeyList[i].worryList.size();j++)
      printf("%llu ", monkeyList[i].worryList[j]);
    printf("] Oper:new=old%c%d, div-by:%d, true:%d,false:%d\n",
        monkeyList[i].oper,monkeyList[i].operValue,monkeyList[i].divisValue,monkeyList[i].trueTo,monkeyList[i].falseTo);
  }
}

void day11printMonkiesPostRound() {
  printf("Monkey List::\n");
  for (auto i=0; i<monkeyList.size(); i++) {
    printf("Monkey %d: ", i);
    for(auto j=0;j<monkeyList[i].worryList.size();j++)
...
void day11printMonkeyInspections() {
  printf("Monkey Inspections::\n");
  for (auto i=0; i<monkeyList.size(); i++) {
    printf("Monkey %d inspected items %d times.\n", i, monkeyList[i].inspections);
  }
}

#define BIGNUM
worryType commonDivisor;

void day11processMonkey(int iM) {
  worryType worryNew;
  worryType argValue;
  worryType curWorry;
  assert(iM < monkeyList.size());
  monkey_t* pM = &(monkeyList[iM]);
  assert(pM->oper=='*' || pM->oper=='+');
  double wn;

  for (auto iWorry=0; iWorry<pM->worryList.size(); iWorry++) {
    curWorry=pM->worryList[iWorry];

    pM->inspections++;

    argValue = pM->operValue;
    if (pM->operValue==-1) {
      // -1 is 'code' for "use old value as arg"
      argValue = curWorry;
    }

    switch(pM->oper) {
      case '+':
        worryNew = curWorry + argValue;
        worryNew %= commonDivisor;
        break;
      case '*':
        worryNew = curWorry * argValue;
        worryNew %= commonDivisor;
        // wn = (double)curWorry * (double)argValue;
        // if (wn != worryNew) {
        //   printf("curWorry=%llu, argValue=%llu, wn=%f : worryNew=%llu\n", curWorry, argValue, wn, worryNew);
        //   assert(false);
        // }
        break;
      default:
        assert("BAD OPERATOR"==NULL);
        break;
    }
    ...
    if (worryNew % pM->divisValue == 0) {
      monkeyList[pM->trueTo].worryList.push_back(worryNew);
    }
    else {
      monkeyList[pM->falseTo].worryList.push_back(worryNew);
    }

  }

  // All worries have been handed to others.
  pM->worryList.clear();
}
...
int main(int argc, char** argv) {
  ...
  vector<string> rawInput;
//  ingestLines("input/day11-sample.input", rawInput);
//  ingestLines("input/day11-sample2.input", rawInput);
  ingestLines("input/day11.input", rawInput);
...
commonDivisor=1;
for(auto i=0; i<monkeyList.size(); i++)
  commonDivisor *= monkeyList[i].divisValue;
#endif
//  for (auto round=1; round<=20; round++) {
  for (auto round=1; round<=10000; round++) {
//  for (auto round=1; round<=1000; round++) {
    for (auto iMonkey=0; iMonkey<monkeyList.size(); iMonkey++) {
      day11processMonkey(iMonkey);
    }

    if (round % 1000 == 0) {
      printf("POST-ROUND-%d:\n", round);
      day11printMonkiesPostRound();
    }
  }

  day11printMonkeyInspections();
  vector<uint32_t> insp;
  for (auto i=0; i<monkeyList.size(); i++)
    insp.push_back(monkeyList[i].inspections);
  std::sort(insp.begin(), insp.end());
  uint64_t top1, top2;
  top1 = insp[insp.size()-1];
  top2 = insp[insp.size()-2];
  // Part 1 answer: 78960
  printf("Monkeybusiness = %llu * %llu = %llu\n", top1, top2, top1*top2);
//  for (auto i=0; i<rawInput.size(); i++) {
  for (auto i=0; i<5; i++) {
      // Read next instruction.
//        sscanf(rawInput[line].c_str(), "%s %d", charCmd, &operand);
